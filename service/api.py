...
# latency 히스토그램 버킷(ms) — p95 산출용. avg만으론 SLO 판단 불가.
_LAT_BUCKETS = (50, 100, 250, 500, 1000, 2500, 5000, 10000)

# 간이 모니터링 카운터 (Ops ②). Redis 스냅샷 저장을 위해 flat 구조 유지.
_METRICS = {"requests": 0, "m5_called": 0, "errors": 0,
            ...
            "prompt_tokens_total": 0, "output_tokens_total": 0,
            "latency_ms_sum": 0.0}
_METRICS.update({f"lat_le_{b}": 0 for b in _LAT_BUCKETS})
_METRICS["lat_le_inf"] = 0


def _observe_latency(ms: float):
    """누적(cumulative) 히스토그램 — le 버킷마다 '이하' 개수를 센다."""
    for b in _LAT_BUCKETS:
        if ms <= b:
            _METRICS[f"lat_le_{b}"] += 1
    _METRICS["lat_le_inf"] += 1
...
def evaluate(req: ExpertResults):
    ...
    if m5 and _state["qwen"] is not None:
        try:
            ...
            _METRICS["prompt_tokens_total"] += int(r.get("prompt_tokens") or 0)
            _METRICS["output_tokens_total"] += int(r.get("output_tokens") or 0)
        except Exception as e:
            _METRICS["errors"] += 1
...
def metrics():
    ...
    lines = [
        ...
        "# HELP m5_prompt_tokens_total Cumulative prompt tokens fed to M5",
        "# TYPE m5_prompt_tokens_total counter",
        f'm5_prompt_tokens_total{{{labels}}} {_METRICS["prompt_tokens_total"]}',
        "# HELP m5_output_tokens_total Cumulative tokens generated by M5",
        "# TYPE m5_output_tokens_total counter",
        f'm5_output_tokens_total{{{labels}}} {_METRICS["output_tokens_total"]}',
        "# HELP m5_latency_ms /evaluate latency histogram (ms)",
        "# TYPE m5_latency_ms histogram",
        *[f'm5_latency_ms_bucket{{{labels},le="{b}"}} {_METRICS[f"lat_le_{b}"]}' for b in _LAT_BUCKETS],
        f'm5_latency_ms_bucket{{{labels},le="+Inf"}} {_METRICS["lat_le_inf"]}',
        f'm5_latency_ms_sum{{{labels}}} {_METRICS["latency_ms_sum"]:.2f}',
        f'm5_latency_ms_count{{{labels}}} {_METRICS["lat_le_inf"]}',

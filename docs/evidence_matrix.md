...
| **합성평가** | **합성(생성) 골든셋**으로 정량 측정됐다 — in-distribution, held-out 아님 |
...
|---|:--:|:--:|:--:|:--:|---|
| 룰 게이트 `compute_emergency_score` | ✅ | ✅ | ✅ | ❌ | `inference/emergency_score.py` · `tests/test_emergency_score.py`(60 **경계·로직** 단위테스트, 임상 아님) |
...
| 프롬프트 압축 (system 슬림 + 시계열 요약) | ✅ | — | ⚠️ | — | **규칙기반**(LLM/LLMLingua 미사용). `_SYSTEM` 200→183토큰(Qwen 토크나이저 실측, 94a64f7 수작업 2줄→1줄 병합), 시계열요약 `_series_prompt` ~41토큰 vs raw 60행≈736. **슬림 단독 Track B 전/후 A/B 없음(무손실 미검증)** — 시계열 개편에 번들 |
...
| FastAPI 서빙 `/evaluate·/health·/metrics·/feedback` | ✅ | ❌ | — | ❌ | `service/api.py` — 엔드포인트 통합테스트 없음 |
| CI (경계60 + Track A mock 게이트) | ✅ | ✅ | — | — | `.github/workflows/ci.yml` · [CI #10 성공, 13초](https://github.com/jinsan02/qwen-llmops/actions/runs/29954017086) |
| CD (GHCR arm64 이미지 빌드·푸시) | ✅ | ✅ | — | ❌ | `.github/workflows/cd.yml` · [CD #1 성공, 3m39s](https://github.com/jinsan02/qwen-llmops/actions/runs/29953228399). **Delivery+이미지 스모크까지만**; 실기 pull·기동 로그 없음 |
| 모니터링 (Prometheus + Grafana) | ✅ | ❌ | — | ❌ | `monitoring/` 설정 파일 존재. **실제 스택 기동·대시보드 스크린샷 없음** |
...
2. **held-out 일반화** — 프롬프트를 평가셋으로 튜닝. 한 번도 안 본 분할·실데이터에서의 성능 미측정.
3. **실운영 데이터** — 피드백·드리프트가 돌 실제 `ai:emergency` 스트림 실적 없음.
4. **서빙/모니터링 통합 실행 증거** — API 통합테스트·Grafana 기동 스크린샷 없음.
